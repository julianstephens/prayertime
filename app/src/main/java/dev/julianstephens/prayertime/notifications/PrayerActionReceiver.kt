...
class PrayerActionReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        val prayerId = intent.getStringExtra(EXTRA_PRAYER_ID)
            ...
            ?: return
        val date = intent.getStringExtra(EXTRA_OCCURRENCE_DATE)
        ...
        when (intent.action) {
            ACTION_COMPLETE -> {
                preferences.saveResolution(prayerId, date, PrayerResolution.COMPLETED)
                scheduler.cancelDelayed(prayerId, date)
                scheduler.cancelWindowClose(prayerId, date)
            }
            ACTION_DELAY -> {
                scheduler.scheduleDelay(prayerId, date, 10)
            }
            ACTION_CANNOT_PRAY -> {
                preferences.saveResolution(prayerId, date, PrayerResolution.CANNOT_PRAY)
                scheduler.cancelDelayed(prayerId, date)
                scheduler.cancelWindowClose(prayerId, date)
            }
        }
    ...
    companion object {
        ...
        fun pendingIntent(
            ...
            return PendingIntent.getBroadcast(
                context,
                PrayerAlarmScheduler.requestCode(prayerId, date, action),
                intent,
